...
void Canvas::draw(Circle &circle, const Color &color)
{
  if (circle.index < 0)
  {
    for (int i = 0; i < 360; ++i)
    {
      ...
      circles.push_back(Buffer(index_circles, Point(circle.center.x + cos(angleToRad) * circle.radius, circle.center.y + sin(angleToRad) * circle.radius), color));
    }
...
void Canvas::draw(const Circle &circle, const Color &color)
{
  ...
  {
    float angleToRad = i * 2.0 * M_PI / 360;
    circles.push_back(Buffer(-1, Point(circle.center.x + cos(angleToRad) * circle.radius, circle.center.y + sin(angleToRad) * circle.radius), color));
  }
}
